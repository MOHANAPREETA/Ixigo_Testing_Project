...
public class Selection_Page extends Searching_page {
	...
	public boolean loadPage() {
		...
		selectRoundTrip();
		enterBoardingPlace("Delhi");
		enterLandingPlace("Mumbai");
		enteringdate("21-09-2025", "25-09-2025");
	...
	public void closePriceLockPopupIfPresent() {
		By[] popupLocators = new By[] { By.xpath("//*[@id='portal-root']/div/div[2]/div/button"),
		...
		for (By pLoc : popupLocators) {
			try {
				...
				try {
					WebElement clickable = new org.openqa.selenium.support.ui.WebDriverWait(driver,
							Duration.ofSeconds(2)).until(ExpectedConditions.elementToBeClickable(pLoc));
					clickable.click();
	...
	public boolean applyDepartureMorning() {
		...
		try {
			...
			Reporter.generateReport(driver, extTest, Status.PASS, "Departure filter applied: 06:00-12:00 (MORNING)");
			return true;
		} catch (Exception e) {
	...
	public boolean applyArrivalMorning() {
		...
		try {
			...
			((JavascriptExecutor) driver).executeScript("arguments[0].click();", el);
			Reporter.generateReport(driver, extTest, Status.PASS, "Arrival filter applied: 06:00-12:00 (MORNING)");
			return true;
		} catch (Exception e) {
			Reporter.generateReport(driver, extTest, Status.FAIL,
	...
	public boolean selectFirstAvailableFlight() {
		...
		try {
			WebElement results = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[text()='Filters']")));
