...
public class Offers_definitions {

	WebDriver driver = Hooks.driver;
	ExtentTest extTest = Hooks.extTest;
	Offers_page optionpage;

	@Given("the user clicks the offer icon")
	public void the_user_clicks_the_offer_icon() {
		optionpage = new Offers_page(driver, extTest);
	...
	public void selects_from_as(String from) {
		...
		from = TestDataContext.excelData[row][0];
	...
	public void gives_the_location_where_as(String where) {
		int row = TestDataContext.currentRow;
		where = TestDataContext.excelData[row][1];
